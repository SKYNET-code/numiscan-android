package com.numiscan.app.ui.theme

import androidx.compose.ui.graphics.Color


/* ---------- Light Theme (Idea 5 refined) ---------- */

val BluePrimary = Color(0xFF1976D2)
val BlueSecondary = Color(0xFF00ACC1)

val LightBackground = Color(0xFFEEF4FA)
val LightSurface = Color(0xFFFFFFFF)

val LightOutline = Color(0xFFD2E2F0)
val LightSurfaceVariant = Color(0xFFE5EEF7)

val LightSuccess = Color(0xFF43A047)
val LightError = Color(0xFFE57373)



/* ---------- Dark Theme (Idea 5 refined) ---------- */

val DarkPrimary = Color(0xFF42A5F5)
val DarkSecondary = Color(0xFF26C6DA)

val DarkBackground = Color(0xFF0B1220)
val DarkSurface = Color(0xFF172235)

val DarkOutline = Color(0xFF30465F)
val DarkSurfaceVariant = Color(0xFF202D42)

val DarkSuccess = Color(0xFF66BB6A)
val DarkError = Color(0xFFEF9A9A)
